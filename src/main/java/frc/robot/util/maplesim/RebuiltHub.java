...
public class RebuiltHub extends Goal {
  ...
  public RebuiltHub(Arena2026Rebuilt arena, boolean isBlue) {
    super(
        ...
        isBlue,
        false); // allowGrounded = false

    this.arena = arena;

    // Custom collision check: sphere radius around hub position
    withCustomCollisionPredicate(pos ->
        Math.pow(pos.getX() - position.getX(), 2)
        + Math.pow(pos.getY() - position.getY(), 2)
        + Math.pow(pos.getZ() - position.getZ(), 2) < Math.pow(GoalRadius, 2));

    // Accept any rotation
    withCustomRotationChecker(Goal.anyRotation());
